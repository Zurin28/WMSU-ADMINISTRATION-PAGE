...
class UniversityBoardSecretary {
    ...
    function deleteOfficial($id) {
        $sql = "DELETE FROM university_board_secretary WHERE id = :id";
        $query = $this->db->connect()->prepare($sql);
...
{
    $sql = "SELECT * FROM university_board_secretary WHERE id = :recordID;";
    $query = $this->db->connect()->prepare($sql);
...
function edit()
{
    $sql = "UPDATE university_board_secretary SET name = :name, title = :title, honorifics_id = :honorifics_id WHERE id = :id;";
    $query = $this->db->connect()->prepare($sql);
